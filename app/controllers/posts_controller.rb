class PostsController < ApplicationController
  ...
  before_action :set_blog_from_user, only: [:new, :create, :edit, :update, :destroy]
  before_action :build_post, only: [:new, :create]
  before_action :set_post_from_user, only: [:edit, :update, :destroy]
  before_action -> { authorize(@post) }, only: [:new, :create, :edit, :update, :destroy]
  ...
  private
    ...
    def post_params
      params.require(:post).permit(:title, :content) if params.has_key?(:post)
    end
end
